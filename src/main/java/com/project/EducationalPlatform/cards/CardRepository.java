...
public interface CardRepository extends JpaRepository<Card, Long> {
	...
	@Query(value = "select * from card c where c.deck_id=?1 and c.learning_day >= c.box_num;",
			nativeQuery = true)
	...
	@Query(value = "update card c set c.box_num = :newBoxNum where c.card_id= :cardId", nativeQuery = true)
	void saveReplanUpdates(@Param("cardId") Long cardId, @Param("newBoxNum") int newBoxNum);

	@Modifying(clearAutomatically = true)
	@Query(value = "update card c set c.learning_day = 0  where c.card_id= :cardId", nativeQuery = true)
