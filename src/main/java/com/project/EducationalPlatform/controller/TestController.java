...
public class TestController {
   ...
   private List<String> stringList;
    ...
    @ResponseBody
    public ModelAndView getFoos(@RequestParam (required = false) Integer contact, @PathVariable(name = "idLevel") int idLevel) {
        ...
        if(contact==null){ //если ответа на тест не было, то просто отображаем форму
          ...
          //  System.out.println("lEv "+ LevelsController.level);
           ...
           /* answerRadio1.setText(stringList.get(0));
            ...
            mav.addObject("questionText",QuestMaker.questions.get(LevelsController.level).get(nowQuestion).getQuestion());
            mav.addObject("stringList",stringList);


        }else{

            if(stringList.get(contact).equals(QuestMaker.questions.get(LevelsController.level).get(nowQuestion).getKey())){
            ...
            if (nowQuestion + 1 != QuestMaker.questions.get(LevelsController.level).size()) {
                ...
                mav.addObject("resText","Правильных ответов: " + correctAnswers+" из "+QuestMaker.questions.get(LevelsController.level).size());
          ...
          /*  mav.addObject("ruList",ru1);
            mav.addObject("deList",en1);
            System.out.println("ОПА! "+contact);
            mav.addObject("answer",contact);*/
        }
